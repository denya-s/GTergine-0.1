...
void InterruptConf(struct tb_event interrupt, int center_x, int quarter3_d_y, bool* P_active, bool* P_cancel, bool exit); // Function declaration.
...
int main() 
{
    ...
    // Set default clear attributes (black background, green foreground).
    tb_set_clear_attrs(TB_GREEN, TB_BLACK);
	...
	while(active)
	{
		...
		tb_print(center_x-14, quarter_d_y, TB_GREEN, TB_BLACK, "| MAIN MENU |");	
		tb_print(center_x-14, quarter_d_y+1, TB_GREEN, TB_BLACK, "|||||||||||||");
	
		tb_print(center_x-18, center_y-2, TB_GREEN, TB_BLACK, "1. Make project");
	 	
		tb_print(center_x-18, center_y, TB_GREEN, TB_BLACK, "2. Load project menu");

		tb_print(center_x-18, center_y+2, TB_GREEN, TB_BLACK, "3. Delete project menu");

		tb_print(center_x-18, center_y+4, TB_GREEN, TB_BLACK, "4 (or Ctrl+C). Exit");
		...
		*P_cancel = false; // Cancel input set to false (don't cancel the input when writing the project name or etc.).
		...
		if(ev.ch == '1') // Create project:
		{
			...
			while (!*P_cancel) // While the input isn't cancelled:
			{
				...
				tb_print(center_x-20, quarter3_d_y, TB_BLACK, TB_BLACK, "                                                              ");						
				tb_print(center_x-20, quarter3_d_y, TB_GREEN, TB_BLACK, "Project name (30 chars max), '/' to stop, 'Ctrl+C' to cancel: ");					
				tb_present(); // Render the screen.
				...
				for (int i = 0; i < 30;) // Check input:
				{
					...
					tb_print(center_x-20, quarter3_d_y+2, TB_GREEN, TB_BLACK, name); // Output each char of the project.
					...
					tb_print(center_x-20, quarter3_d_y+4, TB_GREEN, TB_BLACK, "Chars remaining:"); // Output how much characters the user has left.
					tb_print(center_x-2, quarter3_d_y+4, TB_GREEN, TB_BLACK, remaining); // Output how much characters the user has left.		
				...
				if (*P_cancel)
				{
					...
					tb_print(center_x-20, quarter3_d_y+4, TB_BLACK, TB_BLACK, "                    ");

					// Clear out the currently displayed name buffer. 
					for(int i = 0; i <= size_of_name; ++i)
					{	
						tb_print(center_x-20+i, quarter3_d_y+2, TB_BLACK, TB_BLACK, " ");
					}	
 
					// Clear out "Creating project...".
					tb_print(center_x-20, quarter3_d_y+10, TB_BLACK, TB_BLACK, "                   ");
				...
				{
					// Output the confirmed project name in green, plus a '.cpp' extension at the end.
					tb_print(center_x-20, quarter3_d_y+2, TB_GREEN, TB_BLACK, name); 			
					tb_print(center_x-20+size_of_name, quarter3_d_y+2, TB_GREEN, TB_BLACK, ".cpp"); 
		...
		{		
			...
			tb_print(center_x-20, quarter3_d_y, TB_GREEN, TB_BLACK, "Going the the delete projects menu...");	
		}
...
{
	tb_print(center_x-15, quarter3_d_y+10, TB_WHITE, TB_RED, "Are you sure?"); // Ask for a confirmation.
	...
	{
		tb_print(center_x-20, quarter3_d_y+10, TB_BLACK, TB_BLACK, "             "); // Clear the the confirmation question and dont interrupt.
		tb_present();
	}
}
