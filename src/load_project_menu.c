...
// Helper function to check if a file has a ".cpp" extension:
// Takes a constant char array pointer variable, representing the file name.
int has_cpp_extension(const char *filename) {
    ...
    // If there is a dot and it pointes to a ".cpp" string at the end of the filename - return 0, if not return 1.    
    return (dot && strcmp(dot, ".cpp") == 0);
}
...
void list_c_files(const char *path, int *file_count, char ***filenames, int *filenames_size) {
    ...
    do {
        ...
        // If the current file has a ".cpp" extension, do this: 
        if (has_cpp_extension(name) && (strlen(name) <= 30)) 
        {
    ...
    while (((entry = readdir(dir)) != NULL) && (*file_count <= 10))
    {
       ...
       */
        ...
        // If the current file entry name has a ".cpp" extension at the end, execute this code:       
        if (has_cpp_extension(entry->d_name) && (strlen(entry->d_name) <= 30))
        {
            // Add the filename to the filenames string array on each iteration.
...
*/
    ...
    char *menu_deco_1 = malloc(47 * sizeof(char *)); // Create the menu decoration.
    ...
    // For representing the file numbers in the table. 
    char filenum_to_str[2]={0};

	while(active)
    {
		
        // Main menu graphics:
		tb_print(center_x-14, quarter_d_y+1, TB_GREEN, TB_BLACK, "|||||||||||||||||||||");
		tb_print(center_x-14, quarter_d_y, TB_GREEN, TB_BLACK,   "| LOAD PROJECT MENU |");	
		tb_print(center_x-14, quarter_d_y-1, TB_GREEN, TB_BLACK, "|||||||||||||||||||||");
 
        tb_print(center_x-12, center_y-3, TB_YELLOW, TB_BLACK, "W: 10 projects max");

        tb_print(center_x-26, center_y-2, TB_GREEN, TB_BLACK, menu_deco_1);

        if (file_count == 0)
        {
            tb_print(center_x-26, center_y-1, TB_GREEN, TB_BLACK, "|");
            tb_print(center_x-5, center_y-1, TB_GREEN, TB_BLACK, "NONE");
            tb_print(center_x+19, center_y-1, TB_GREEN, TB_BLACK, "|");
            tb_print(center_x-26, center_y, TB_GREEN, TB_BLACK, menu_deco_1);
        } else 
        {
            for (int i = 0; i < file_count; ++i)
            {
                //Convert the file numbers to a fancy string format to output them.
                if (i+1 < 10) 
                {
                    sprintf(filenum_to_str, "0%d", i+1);
                }
                else 
                {
                    sprintf(filenum_to_str, "%d", i+1);
                }

                tb_print(center_x-21, center_y-2+spacer-1, TB_RED, TB_BLACK, filenames[i]);
           
                tb_print(center_x-26, center_y-2+spacer-1, TB_GREEN, TB_BLACK, "|");   
                tb_print(center_x-25, center_y-2+spacer-1, TB_GREEN, TB_BLACK, filenum_to_str);
                tb_print(center_x-23, center_y-2+spacer-1, TB_GREEN, TB_BLACK, "|"); 
                tb_print(center_x+19, center_y-2+spacer-1, TB_GREEN, TB_BLACK, "|");
            
                tb_print(center_x-26, center_y-2+spacer, TB_GREEN, TB_BLACK, menu_deco_1);
       
                // Increment the spacer so it puts the menu decoration into the correct spot.
                spacer+=2;
            }	
        } 
        
        tb_print(center_x-26, center_y+spacer+1, TB_GREEN, TB_BLACK, "Input the project file you would like to load (^C to exit):");

        tb_present();
       
        tb_poll_event(&input);

        // Space for creating the project opener:
